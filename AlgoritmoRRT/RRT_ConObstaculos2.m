...
tolerancia = 4;         % tolerancia: Distancia máxima al objetivo para considerar que se ha alcanzado. Si un nodo está a 5 unidades o menos, se considera un éxito.
max_iter = 2000;        % max_iter: Número máximo de intentos para encontrar un camino. Si no lo encuentra en 1000 intentos, se detiene para no quedarse en un bucle infinito.
...
w_obs = 1;         % w_obs: El ancho del obstáculo.
...
for i = 1:max_iter % El bucle se repite hasta 1000 veces. Si encuentra la solución antes, se detiene.
    ...
    if ~verificar_colision(nearest_node, new_node, x_obs, w_obs, y_obs) % Se mandan los valores de el nuevo nodo, el nodo mas cercano, tamaño del "paso" y valores del obstaculo
