function Ejecutar_RRT_Binaria_V5()
    global obstaculos punto_inicio punto_final ax;

    % Parámetros V5 adaptados al lienzo de 500x500
    x_limites = [0, 500]; y_limites = [0, 500];
    inicio = punto_inicio; objetivo = punto_final;
    tolerancia = 20; max_iter = 7000; step_size = 15;
    prob_objetivo = 0.05; grid_size = 25; prob_bias = 0.5;

    num_celdas_x = ceil(500 / grid_size);
    num_celdas_y = ceil(500 / grid_size);
    ...
    for i = 1:max_iter
        ...
        % Muestreo (Bias de Objetivo y Densidad)
        r_prob = rand;
        ...
        % Verificación de colisión (usando la función adaptada V5)
        if ~Verificar_Colision_V5(nearest_node, new_node, obstaculos)
            nodes = [nodes; new_node];
            parent(end+1) = idx;

            % Actualizar mapa de densidad
            c_idx = max(1, ceil(new_node(1)/grid_size));
            r_idx = max(1, ceil(new_node(2)/grid_size));
    ...
    if is_connected
        ...
        Exportar_Reporte_Metricas_V5(path, tiempo_busqueda, size(nodes,1), size(path,1), iteraciones_totales, nodos_colisionados);
    else
