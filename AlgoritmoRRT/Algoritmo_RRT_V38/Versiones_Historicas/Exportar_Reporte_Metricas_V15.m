% Exportar_Reporte_Metricas_V15.m
% =========================================================================
% === FUNCION PARA CALCULAR COSTO Y GENERAR ARCHIVO DE RESULTADOS ===
% =========================================================================

function Exportar_Reporte_Metricas_V15(path_final, t_busqueda, t_optimizacion, n_total_arboles, n_camino, n_iter, n_colisiones)
    % 1. Calcular la longitud (costo) total de la ruta definitiva.
    costo_final = 0;
    for k = 1:size(path_final, 1) - 1
        costo_final = costo_final + norm(path_final(k+1, :) - path_final(k, :));
    end

    % 2. Configurar rutas de guardado apuntando a la carpeta de Resultados
    ruta_del_script = fileparts(mfilename('fullpath'));
    % Se asume que Versiones_Historicas está en la misma carpeta raíz
    ...
    nombre_archivo = ['Metricas_V15_', datestr(now, 'HHMMSS'), '.txt'];
    ruta_destino = fullfile(nombre_carpeta, nombre_archivo);

    % 3. Escribir el archivo.
    fid = fopen(ruta_destino, 'w');
    if fid ~= -1
        fprintf(fid, '=== REPORTE DE CALIDAD RRT* V15 ===\n');
