...
for i = 1:max_iter % El bucle se repite hasta el maximo de iteraciones definido. Si encuentra la solución antes, se detiene.
    ...
    if ~Verificar_Colision_Segmento(nearest_node, new_node, obstaculos, distancia_muestreo) % Se llama a la funcion verificar_colosion y se mandan los valores de: el nodo mas cercano, el nuevo nodo y valores del obstaculo
        ...
        if norm(new_node - objetivo) < tolerancia   % Comprueba si el nuevo nodo está lo suficientemente cerca del objetivo. `norm` calcula la distancia entre los dos puntos.
            ...
            % IMPORTANTE: Debe cargar o definirse la función 'Optimizar_Ruta_Final'
