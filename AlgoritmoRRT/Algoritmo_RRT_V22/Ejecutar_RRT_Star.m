...
if is_connected
    ...
    % ----------------------------------------------------------------------
    % === 1. RECONSTRUIR EL CAMINO INICIAL ===
    % ----------------------------------------------------------------------
    ...
    costo_inicial = 0;
    for k = 1:size(path_coords_inicial_completo, 1) - 1
        costo_inicial = costo_inicial + norm(path_coords_inicial_completo(k+1, :) - path_coords_inicial_completo(k, :));
    end
