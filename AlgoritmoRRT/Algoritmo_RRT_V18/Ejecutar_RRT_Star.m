...
if is_connected
    ...
    % ==================================================================
    % === CÁLCULO DE COSTO Y EXPORTACIÓN ===

    % 1. Calcular la longitud (costo) total de la ruta roja final
    costo_final = 0;
    for k = 1:size(path_final_coords_completo, 1) - 1
        % Suma la distancia entre cada punto de la ruta definitiva.
        costo_final = costo_final + norm(path_final_coords_completo(k+1, :) - path_final_coords_completo(k, :));
    end

    % Obtiene la ruta de la carpeta donde está este script actualmente
    ruta_del_script = fileparts(mfilename('fullpath'));

    % Construye la ruta subiendo un nivel de forma segura
    % fullfile se encarga de poner las diagonales correctas (\ o /) según tu Windows
    ...
    % Crear el nombre del archivo y la ruta final
    nombre_archivo = ['Metricas_V18_', datestr(now, 'HHMMSS'), '.txt'];
    ruta_destino = fullfile(nombre_carpeta, nombre_archivo);

    % Intentar abrir el archivo
    fid = fopen(ruta_destino, 'w');

    % --- ESCRITURA DE LOS DATOS (Del reporte actual) ---
    fprintf(fid, '=== REPORTE DE CALIDAD RRT* V18 ===\n');
    fprintf(fid, 'Ruta de guardado: %s\n', ruta_destino);
    fprintf(fid, '-----------------------------------\n');
    fprintf(fid, 'Tiempo Búsqueda Inicial: %.4f s\n', tiempo_busqueda_inicial);
    fprintf(fid, 'Nodos totales en los dos árboles (al conectar): %d nodos\n', total_nodos_encontrados);
    fprintf(fid, 'Tiempo Optimización: %.4f s\n', tiempo_optimizacion);
    fprintf(fid, 'Nodos en el camino final optimizado: %d nodos\n', nodos_camino_final);
    fprintf(fid, 'Longitud de Ruta (Costo): %.2f unidades\n', costo_final);
    fprintf(fid, 'Iteraciones ejecutadas: %d\n', iteraciones_totales);
    fprintf(fid, 'Colisiones Detectadas: %d\n', nodos_colisionados);


    % Cálculo de la eficiencia: (Éxitos / Total de intentos)
    eficiencia = (1 - (nodos_colisionados/iteraciones_totales)) * 100;
    fprintf(fid, 'Eficiencia de Muestreo: %.2f%%\n', eficiencia);

    fclose(fid); % Cierra el archivo para asegurar que se guarde en el disco.
