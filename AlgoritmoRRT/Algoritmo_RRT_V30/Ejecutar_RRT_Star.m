...
step_size = 14;        % La longitud de cada paso del algoritmo.
...
% Cargar el Mapa Discreto (Grid Caching) en memoria
disp('Cargando Grid Caching en memoria...');
mapa_discreto = load('Mapa_Discreto.txt');

% Cargar el Esqueleto y extraer las coordenadas seguras
disp('Cargando Eje Medial (Esqueleto) en memoria...');
mapa_esqueleto = load('Mapa_Esqueleto.txt');

% La función 'find' nos da las filas y columnas exactas donde hay un 1 (centro de calle)
[filas_esq, cols_esq] = find(mapa_esqueleto == 1);

% Restamos 1 porque MATLAB es base-1 (1 a 501) y nuestro plano es base-0 (0 a 500)
...
for i = 1:max_iter
    ...
    % --------------------------------------------------------------------------
    rand_point = Generar_Punto_Aleatorio(nodo_meta_bias, mapa_densidad_global, x_limites, y_limites, grid_size, prob_objetivo, prob_bias, coordenadas_esqueleto);
