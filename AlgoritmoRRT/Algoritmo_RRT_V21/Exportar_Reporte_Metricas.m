...
function Exportar_Reporte_Metricas(path_final, t_busqueda, t_optimizacion, n_total_arboles, n_camino, n_iter, n_colisiones)
    ...
    nombre_archivo = ['Metricas_V21_', datestr(now, 'HHMMSS'), '.txt'];
    ruta_destino = fullfile(nombre_carpeta, nombre_archivo);

    % 3. Escribir el archivo.
    fid = fopen(ruta_destino, 'w');
    if fid ~= -1
        fprintf(fid, '=== REPORTE DE CALIDAD RRT* V21 ===\n');
